...
/**
 * The existing Android health path, isolated behind the provider contract.
 *
 * aggregate() is deliberately retained for steps: summing raw records can
 * double-count when more than one source writes overlapping intervals.
 */
    ...
    override suspend fun readSteps(date: LocalDate): HealthRead<Int> =
        withContext(Dispatchers.IO) {
            ...
            if (!hasPermission(HealthMetric.STEPS)) {
                return@withContext noPermission()
            }
