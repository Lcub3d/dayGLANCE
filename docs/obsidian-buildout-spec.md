...
**Correction record (2026-09-06, the night of the SSE re-arm) — the inbox exception keeps the inbox copy's RECORD, and an inbox task scheduled onto another day gets its date prefix.** Six inbox tasks scheduled on the Mac for the next day flipped back to the inbox fleet-wide within a minute, and one line lost its middle to an editor merge on the way. Two defects, one incident. *(a)* The writeback decided "rescheduled to another day" by comparing the task's date with its snapshot date, and an inbox task's snapshot has none — so the six lines got a time and NO inline date prefix, and every parser read them as today (`dateChanged` in the writeback now falls back to the note's own date; the writer clears the prefix when a task moves back onto the note's day, `noteDate` in `updateTaskLines`; harness scenario 13). *(b)* A second desktop's plugin reported the rewritten note before that desktop's DB pull had delivered the schedule, so its merge met a timed line with an inbox copy. The user-move rule kept the task in the inbox — correctly, that rule is what protects a fresh unschedule from a stale read of the still-timed line — but built the inbox copy FROM THE TIMED LINE: an inbox record carrying a date and a start time, the war shape `buildExistingObsidianTaskContext` already names. That record differed from the stored copy, re-stamped with a fresh `lastModified`, pushed, and outranked the real schedule. **The ruling (three options weighed: vault wins on a timed line — fixes this and reintroduces the bounce; judge by statement time — right both ways, needs an mtime observations do not always carry; keep the decision, fix the record): the inbox exception keeps the stored inbox copy's shape, byte for byte.** No winner changes; the losing side simply stops fabricating a timestamp, the same defect class as the phantom re-stamp of the same day, and the DB tier delivers the answer on the next pull. The mirror case is a no-op for the same reason. Harness scenario 14 replays the sequence on a second device; `obsidian.inboxRecord.test.js` pins the record. *A third defect surfaced in the ruling's first test run:* the legacy hint (`obsidianLegacyId`) is recomputed from the line's content AND date every parse, so an inline date prefix, or a note moved to another path, flips it on a task already matched by its id — the stored copy said one hint, the re-parse another, the compare read an edit, the task re-stamped (the three App Store Connect rows ping-ponging after their note moved were this). An id-matched task now keeps the hint it has; a copy with no hint takes the parsed one, because the vault-wide deletion detector reads hints off the scan to rescue a stamped legacy key (`keepExistingHint`). That closes the first piece of the legacy-hint item; the plugin's duplicate-token dedupe scope remains. *Recorded beside this, unresolved:* a time typed BY HAND in Obsidian onto a task that sits in the inbox is silently ignored until the app schedules it — the time prefix is not among the fields the vault-edit adoption (ruling 2) covers. Whether it should schedule the task or be visibly refused is its own ruling (§8); "nothing happens" is not the answer either way.

**6. Existence divergence is judged at the vault's statement time — the note's mtime.** (Revival stamping, Phase 6; `reviveScannedAgainstTombstone` in `utils/mergeObsidianTasks.js`.) **The tombstone question is an EXISTENCE question, and the honest timestamp for "does the vault say this line exists" is the note's mtime — the vault's statement time. The task record's `lastModified` answers a different question: content LWW. Those two questions happened to share a field, and that is what made the documented "a later re-creation in Obsidian wins" rule nominally true but mechanically inert for tasks** — a fresh import carries an epoch `lastModified`, so a line deleted and retyped verbatim (same content hash; or, for a stamped task, the deterministic `deriveBlockId` re-minting the tombstoned token — the come-back-then-vanish flicker) lost to its tombstone until the 60-day GC, a latent direct-mode trait that note-scoped inference (the availability note below) made common. The ruling: when a SCANNED row — a line the batch actually read out of its note — would be dropped by a tombstone OLDER than that note's mtime, the row is admitted with `lastModified` lifted to the note mtime, in that one case only. The lift is what makes the revival propagate: the revived row out-LWWs its tombstone at the `applyEngineData` gate, the `mergeSync` cleanse, and the push guard with no changes to any of them. **This makes the documented rule actually true, which is ruling 4 (the vault controls existence) working rather than being amended.** Two recorded rejections, because both will be proposed again: *(i) stamping every fresh import with its note's mtime* — epoch on fresh imports is LOAD-BEARING: a bare re-parse carries none of the app-only fields (projectId, color, notes, assignedUserSyncIds, completedAt), so an mtime-stamped default-filled copy from a cold-open or vaultless device would beat another device's real record in row-level LWW and wipe those fields fleet-wide; epoch guarantees a parse never beats a record — don't change it, and note that WIDENING the revival condition beyond "tombstone exists AND the note was written after it" reintroduces exactly this hazard; *(ii) clearing the tombstone key on reappearance* — `deletedObsidianKeys` merges grow-union, newest-per-key, so a local removal is re-added by every other device's copy; removals structurally don't stick in that channel, and out-timestamping the tombstone is the only durable win. Retained rows are deliberately not revived (their notes are not in the batch, so there is no fresh vault statement to lift from); a stamped task transiently dropped that way returns on its note's next observation, which the stamp's own write guarantees exists. *(Amended 2026-09-05, audit M10, the revival half: the evidence is REAL mtimes only. Two writers fabricated one — the native scan falls back to "now" for a bridge build that reports no mtime, and the inbound module falls back to the observation time for a note reported without one — and either, read as the vault's statement time, is always "after the deletion": every tombstoned row in such a note revived on every scan. The text stamp keeps its fallback (it is a note-text LWW stamp, and was never wrong as one); the revival map is now built separately from real mtimes alone (`noteMtimes` on the scan and observation results), and a note without one is no revival evidence at all — the row stays gone until a device with a real mtime sees it. The deletion-stamp half of M10, a missing observation mtime making a deletion stamp read as observation time, is a plugin matter — the plugin always reports the mtime — and a deleted note is deliberately stamped at its sighting.)*
...
**The seventh record — THE DB-TIER RETIRE/TOMBSTONE WAR (2026-08-31, the same night's third incident; the tripwire exercised).** The fragments' cleanup collided with their retirement bookkeeping and produced a war one tier DOWN from the vault-file wars: dayGLANCE's own DB sync, at ~2s a round while Obsidian ran. Three interlocking pieces, each per-spec in isolation, verified against logs and the engine source: **(1) the resurrection primitive** — the engine's push has no delete POLARITY: `pushDirtyRows` re-classifies each dirty id at push time by mirror presence, the pull runs BETWEEN the snapshot diff that authorized a delete and that push, and a push never advances the pull cursor, so the device's own pushed rows re-list on every pull — the pull re-supplied the "deleted" row into the mirror and the blessed delete silently flipped into an upsert (`written:2 deleted:0` on a delete-only dirty set; the client's own WRITE LOOP detector named the `upsert -> delete -> upsert` shape); **(2) the re-mint edge** — stamp-on-sight re-derived the SAME dg successor for each resurrected legacy row (determinism is the unanimity feature everywhere else), re-recording the retirement and re-creating a successor row the tombstones killed at the next apply boundary; **(3) the trigger fuel** — each round's writes produced seqs the maxSeq-only own-ack ring couldn't cover, so SSE nudges ran the loop at trigger speed (closing Obsidian froze it to poll cadence — the plugin's intent-consumption deletes were the metronome). Fix 4's retirement-heal breaker never fired: it guards the SKIPPED/heal arm, and these rows propagated as `retired`/`tombstoned` every cycle. *The fixes:* **the re-mint refusal** at the minting site (`isTombstonedRemint`: a derivation whose retirement is already on the record with the successor tombstoned-not-live is a completed identity move, not a new one — refuse loudly; a live successor or a tombstone-less absence still mints); **the polarity reassert** post-pull (`reassertPropagatedDeletes`: weigh each pull-resupplied copy against the evidence that authorized its delete — tombstone still at-least-ties → evict the copy so the push stays a delete; genuinely newer → the flip IS correct newest-write-wins); **the delete-propagation streak latch** (the same id delete-propagated a 4th time in 10 wall-clock minutes latches for the session — the shape-agnostic wall for the propagate arm, so the NEXT composition of correct rules hits it without needing its own diagnosis first); and **own-ack coverage for every seq** (the server emits one nudge per batch but one per `deleteRow`; the engine's client now records each delete ack's seq, not just the folded max). *Addendum (2026-09-05, the stray row): the apply-boundary gate's third face had a hole of its own. Dropping a tombstoned-and-older pulled row kept it out of state, but a device that never admits a row never snapshots it, so its diff never sees it vanish, the guard never propagates a delete, and the cursor moves past it — the VAULT row outlived every device's tombstone, invisible to dayGLANCE and visible to anything reading the vault directly (the plugin's sidebar mirror showed a task dayGLANCE had dropped a day earlier; a sidebar completion on it was then held for the seven-day stale window). The gate now drops AND marks the entity dirty, the ghost-containment shape: absent from the mirror, the id is pushed as a soft-delete the same cycle. Rows already past every cursor are reachable only by hand — `window.__dayglance.purgeVaultRow('tasks:<id>')` (mark dirty, one cycle) or `window.__dayglance.resyncVault()` (cursor reset and reload, so the full re-pull runs every stray through the gate) — documented in the companion spec 4.2. Pinned by the echo-war suite's stray-row case.* And per the Phase-7 assessment's tripwire — three wars in one night is a new class, not a named leftover — **dayGLANCE's SSE nudge consumption now defaults OFF** (`sseNudgesEnabled`, localStorage escape hatch `dayglance-sse-nudges`), reverting to the 5-minute/2-minute polls and the plugin's 30-second drain until these breakers are proven against live traffic; the plugin's own SSE stays, since every war cycle must pass through dayGLANCE's drain. *Addendum (2026-09-05, audit M9, the fuse): the refusal needs both legs on the record, and each pruned at the same 60-day window keyed on its OWN timestamp — the retirement's `retiredAt`, the successor's `deletedAt`. The tombstone is stamped after the retirement it kills, so the retirement aged out FIRST, the refusal disarmed while the tombstone and the untagged line still stood, and the oscillation could resume, damped only by session latches. Now an aged retirement is kept while a tombstone that itself survives the cutoff names its direct successor (`pruneRetiredTaskIds` takes the post-prune bundles; both transports pass the same ones, so the lockstep holds): the two legs prune together, at the tombstone's age. Bounded, and the end state is convergence, not the war — with the refusal armed nothing re-stamps the tombstone, so at 60 days from the kill both go and the next mint creates a successor no tombstone kills. Pinned in `retiredTaskIds`, `tombstoneRetention` and `mergeSync` tests.* *Addendum (2026-09-05, the phone-soak duplicate — a delete/resupply loop the war guard froze as a visible duplicate, one task on the timeline AND in the inbox on the Mac): the resupply was the apply's rescue pass (`utils/rescueUnsyncedTasks.js`, the ala7ur flicker fix that made Obsidian tasks rescuable). It runs per list and saw only its own list: a cross-list move made on the phone (inbox → scheduled) arrived as "the id is gone from the inbox result", the rescue found the old inbox copy in prev — rescuable, untombstoned, since a move leaves no tombstone — and put it back, undoing the reconcile's choice every cycle; the next diff saw it as new, the push soft-deleted its row, the reconcile dropped it again, the rescue re-added it. Three rounds in ten minutes tripped the guard, which kept both copies — exactly its job, and exactly the tell. The rescue now takes the ids live ANYWHERE in the incoming result (both lists and the recycle bin, the same set the retirement pass uses) and never rescues one: an id the merge placed somewhere is governed by the merge, and its absence from THIS list is a move, not a loss. Not phone-specific: any Obsidian task scheduled from the inbox on one device while another device still held the inbox copy. Closed before the SSE re-arm on purpose — at trigger speed this shape is the war class.* *Addendum (2026-09-05, audit M5 and the cursor gap): two smaller faces of the same resurrection primitive. (M5) Because a push never advances the pull cursor, this device's own pushed rows re-list on the next pull — and the wrapper invalidated the row's push ack on EVERY applied pull row, its own echo included. Under a withheld snapshot the stale baseline then re-marked the row dirty, so an insert-only row (a singleton bundle, a recurring template — kinds whose merge with their own echo is the identical set) re-pushed once, identical, seq-advancing, every other cycle. The ack now survives when the applied mirror row hashes to exactly what was acked — the same hash the snapshot uses — and drops only on real remote content. (The cursor gap) The device-cursor report read the live HWM and ran BEFORE the commit; a commit-phase throw then rolled the HWM back, leaving the server's `lastSeenSeq` ahead of what this device had kept, and a tombstone the server GC'd in that gap was never re-pulled. The report now runs after the commit and the snapshot save, so the server never believes this device consumed more than it committed. Both pinned in the wiring suite.*
...
1. **Pairing and credentials** — the 3.12 dead-drop both sides (`bridgePairing.js` in the format package: wire format, PBKDF2 offer key, HKDF subkey with the per-pairing salt; `src/utils/obsidianBridgePairing.js` app-side flow; the plugin's `pairing.ts` modal + verification), the pairing panel in dayGLANCE settings, and the GLANCEvault client in the plugin (bundled `vaultClient.js` behind an Obsidian `requestUrl` fetch shim — CORS-free on desktop and mobile). Ends with a plugin that is paired and can talk to GLANCEvault but moves no data; pair, confirm, revoke, re-pair are each testable. The bridge stream will live in the app-scoped GLANCEvault namespace `dayglance-bridge`, fully separate from dayGLANCE's own rows. *(Amended 2026-09-05, the mobile transport gap: the APP side's bridge modules — stream, inbound, actions — built their GLANCEvault client on global fetch, which is the WebView's CORS-bound fetch on Android (`appassets.androidplatform.net`) and iOS (the `dg://` scheme); the DB sync engine and the intents transport had routed through the native HTTP bridge and the Electron proxy for exactly this reason. On the phones every bridge request died at the CORS wall: no cached pairing meta, so every emit was refused and each daily-note edit or tagged task raised "the bridge queue is unavailable"; no observations or sidebar actions consumed while paired, only DB sync. One client factory now serves all bridge modules over the native-safe fetch (browser and harness keep global fetch). A direct-write fallback was considered and rejected: a device whose vault copy carries a fresh plugin heartbeat must not write the vault itself, whatever the state of its own stream — that is the single-writer rule of §3.2, and the queue is durable precisely so a device can wait. The sync toast also gained tap-to-dismiss: an error cleared only on the next successful cycle, and on a phone every edit re-raised it.)* *(And the 2026-09-06 iOS hang, closed the same day: the iOS shim is a Proxy that answers ANY method name, so the web layer's feature check for the asynchronous daily-note scan (`getAllDailyNotesAsync`, Android's contract) passed on iOS while the scheme handler had no such case — the call answered `null` at once, the callback never fired, and the direct-mode cycle awaited it forever with its in-progress guard held: a spinner that never finished and no further Obsidian sync until a force-quit, reachable whenever Obsidian had been backgrounded long enough for the heartbeat to go stale. iOS now implements the method (the scan off the main thread, the same JSON delivered through `__obsidianDispatch`), and the web layer bounds the wait at thirty seconds and rejects — surfaced and released like any scan error — so no bridge can hold a cycle hostage again. The wider lesson is recorded beside it: on a phone, a stale heartbeat also means a stale vault copy, so the direct-mode fallback there scans exactly the copy that cannot be trusted; whether it should fall back at all is an open ruling.)*
...
3. **Arbitration and handoff** — `pluginAuthoritative` consumed, direct write paths stop per 3.2, the Settings mode indicator, and the handoff hazards closed. As built, with the two gate rulings:
   ...
   - One inbound source per device: authoritative → the observation stream (through the shared scan pipeline, cursor committed after apply), never the direct scan; anything else → the scan. **Accepted cost, corrected and narrowed:** with archive-and-reconcile, deletions made in the vault while a device is paired DO propagate — at the exit, through the reconcile. What still lingers is only (1) a task created AND deleted entirely within the paired window (never in any baseline; the plugin's `deleted` observations are deliberately not used — rename-vs-delete event semantics are exactly what the detector was built to avoid), and (2) the residue of an archive that expired past the 60-day horizon before a clean reconcile (logged when discarded). Detection of new deletions resumes with direct mode as before. *(Amended 2026-09-05, audit low: a plugin-mode cycle whose fetch reads nothing — braked, unreachable, or no pairing record — is not a successful sync. It used to finish green, stamping "last synced" and clearing the task-write latch on a dead stream. It now leaves the stamp where it was, keeps the latches, and shows the state through the sync error channel; the next cycle that reads the stream clears it through the shared finish. The inbound module remembers why the last fetch produced nothing (`lastBridgeInboundFailure`) so the rate-limited case reads as a pause, not a fault.)*
...
**Device-local plugin state (2026-09-05, the post-soak plugin batch; the HWM_PERSIST_GAP investigation's answer).** The data.json churn behind the §3.2 incident was never the persist gap: `data.json` is ONE fleet-shared file by the per-vault pairing ruling (Obsidian settings sync ships it to every copy), and the bridge cursor, the applied-intent set, the adopted scope, the linked-note map and the device id are per-COPY volatile state that every copy saved into that shared file — Obsidian Sync then held several versions differing only in bookkeeping, and a settings flip on one desktop could lose to another copy's concurrent cursor save. Persist-on-intent-only cut the frequency; it did not change the shape. The volatile state now lives in Obsidian's vault-scoped device-local storage (`App.loadLocalStorage` / `saveLocalStorage`, 1.8.7+; `src/localState.ts`), and `data.json` keeps what is legitimately shared: the pairing, the settings, and the config-row cache — written only when that cache's VALUE changes. Effects: each vault copy has an honest cursor and applied set (a copy starting fresh re-lists only unconsumed rows, since applied intents are deleted from the stream, and re-applies nothing that was applied on its own files elsewhere); the device id is truly per copy, which is what the heartbeat's wire key always meant; the settings-loss hazard closes. Migration is one-time and pure (`migrateToLocalState`, unit-pinned): the first load seeds the local store from the data.json fields and strips them; a copy on an older build writes them back until it upgrades (main.js ships with the settings, so the fleet upgrades together) and a newer build strips them again — rollout churn only. An Obsidian without the API keeps the old shape unchanged. *Latent bug found on the way, fixed:* the drain's persist REPLACED the state object without `linkedNotes`, so every intent persist dropped the linked-note map (`stateSnapshot` now carries every transport-owned field to every persist site; scenario R4). *The rest of the batch, all pinned in the scope scenarios (R1–R5):* **M4** — armed-but-unreported observations were memory-only, so a plugin reload (an update, a mobile webview kill) dropped the last edit's report until the note's next touch, and the heartbeat resumed fast enough that direct-mode fallback never covered it; the pending paths persist (device-locally, so the persist is free) and re-arm on construction, a vanished path reporting the deletion instead. **M12** — a deleted report's failed retry coalesces per path with a recreated file's pending observation, and the deleted path skipped the `exists` re-check the live path had, so the stale retry could report a standing note as gone; a deleted report now re-checks and reports the note as it stands. **Unsupported intents** — the applied-set fast path deleted the rows the leave-for-newer-builds policy meant to preserve whenever a retryFloor clamp re-listed them; unsupported ids are tracked (persisted) and their rows are never cleaned up here. **SSE connect timeout** — a hung connect occupied `sseReq` forever with no failure path; twenty seconds without response headers now takes the same reconnect-with-backoff path a refused connect takes (desktop-only Node http, manual verification). **The pairing-meta ack** — `publishPairingMeta` lives in the plugin and its write's seq was never recorded as our own, so its SSE echo woke one idle drain per publish; it returns the seq now and every caller records it.
...
- **Idle-nudge cost** (the shared account seq carries other apps' activity): one empty app-scoped list per debounced foreign nudge — the drain is its own probe, since `/sync/:app/list` returns only bridge-namespace rows. *(Done 2026-09-05.)* glance-vault's activity frames now carry `app` (sync batch and soft-delete tag their route namespace, intents inserts tag `intents`, `ready` stays seq-only), and both consumers filter on it: the plugin's nudge gate drops a foreign-namespace nudge with the cursor advanced and no drain, so a foreign nudge now costs nothing; dayGLANCE's coalescer routes each fan-out kind to the namespace it reads — the DB sync drain to the app's own rows, the intents drain to intents inserts, the Obsidian observation cycle to the bridge namespace — so a sibling app's write, or a bridge observation from a phone, costs the DB tier nothing (the Obsidian cycle first, 2026-09-05; the DB and intents drains the same day, owner's call). THE MISSING-TAG CONTRACT, on both sides: a frame without `app` is "unknown" and behaves exactly as before the tag — an older server keeps working after these clients update, which matters for a fleet that does not update simultaneously. What was NOT deleted: dayGLANCE's #1494 probe (`pendingBridgeObservations`). The tag is namespace-grained and the bridge namespace carries more than observations — a peer device's intent emissions and the plugin's own intent-row soft-deletes are bridge-tagged too — so the probe remains the row-level layer beneath the tag on the matched path, and the whole gate for an untagged frame; removing it would make those bridge writes wake read-only cycles the probe filters today.
...
**Field finding (2026-09-06): the phantom re-stamp, and the carry inverted.** Day one of the SSE re-arm on the Mac, the phones polling. A sidebar completion of a task that had been added to the daily note and then scheduled onto the timeline in dayGLANCE vanished twice: the `act:` row was written and consumed each time, the Mac applied it, and three minutes later the Mac pulled the same task back UNCOMPLETED with a newer `lastModified` from an Android whose Obsidian was backgrounded (direct mode, stale vault copy: its line was still untimed and unchecked). Foregrounding Obsidian on the phone ended it. The mechanism, pinned on the real pipeline (`obsidian.staleLineRestamp.test.js`): the scan/observation merge rebuilds every task from its line and carried only an ALLOW-LIST of app fields, and `stampTimestamps` reads any other difference against the stored copy as an edit and fabricates a fresh `lastModified` — the exact hazard its own header warns of. Two keys were proven. `transitionId`: every completion mints one; every re-parse dropped it, so the first re-parse after any completion re-stamped the task on every device that parsed the line, the Mac included via its own writeback's observation. `priority`: scheduling from the inbox STRIPS the key, and an untimed line re-parses inbox-shaped with `priority: 0`, so a phone holding a stale untimed line re-stamped the scheduled copy every time it pulled a newer desktop copy — every desktop edit was outranked within one phone cycle, and a completion landing inside that window was overwritten. The completion OR in the merge could not help: it protects a copy that already holds the completion, and the phone's copy never received it.

**Why the mechanism changed rather than the list.** `energy` was found in the same structural position while tracing the two — app-set, not line-derived, dropped by every re-parse, wiping silently — and that made three found one incident at a time: `archived`/`completedAt` (the cold-open churn), `assignedUserSyncIds`, `transitionId`, `priority`, `energy`. An allow-list drops everything not on it by default, so the next app-side field lands in the same hole and is found when something goes wrong months later. An audit of what the app sets on tasks against what the carry listed, made before merging either shape, found the list was not a latent risk but a cost already being paid:

| Field | Set where | What every re-parse did to an Obsidian task |
|---|---|---|
| `focusMinutes` | focus sessions accrue on scheduled tasks | reset the minutes — a number nobody watches because they expect it to grow |
| `bucketId` | bucket lists on inbox tasks | dropped the task out of its bucket back to the inbox — dayGLANCE randomly un-filing things |
| `hyperglanceSessionDate`, `createdAt` | session task creation | lost the session grouping once the task was written to the vault (any task written to a note becomes an Obsidian import on commit) |
| `transitionId`, `energy` | every completion; the energy axis | the phantom re-stamp above; energy wiped |
| `openInInbox` | creation flag | harmless |

(`obsidianNoteMissingAt` and `order` live on projects and goals, `ownerSyncId` on routines and habits, `keepUnscheduled` is modal state never persisted on the task, `taskHeading` is config; nothing strips unknown keys on load, so the re-parse was the only place these died.)

**The carry is now BY EXCLUSION** (`preserveObsidianAppFields`, `utils/mergeObsidianTasks.js`, moved out of the hook and tested where the merge is): every field on the app's copy survives a re-parse UNLESS the line owns it. The line-owned set is `LINE_OWNED_TASK_FIELDS`, exported by `@glance-apps/obsidian-format` beside the parser that emits it, plus the two the per-note merge adds (`lastModified`, `projectId`). A line-owned key absent from the scan is CLEARED, never carried — an un-scheduled line drops the schedule keys, a stripped token drops the block id — which is what keeps the inversion from re-creating the war shape of an inbox record carrying a stale `startTime`. The explicit rules that remain are real semantics on line-owned keys, not a memory list: `completedAt` (app wins when it has a value; the vault marker fills the blank), `projectId` and `deadline` (adopted on a vault edit with an explicit null on removal, so absence means "no opinion" and the app's value fills it), the `priority` SHAPE (absent and 0 are one state; the app's shape wins, and `stampTimestamps.normalizeField` reads them as equal on the compare side), and `transitionId` riding with the completion state (never onto a completion the vault just made). **The tests are the durable part:** (1) the KEY CONTRACT — the parser (`taskLines.contract.test.js`, in the package) and the parser plus the per-note merge (`obsidian.lineOwnedFields.test.js`, in the app) emit EXACTLY the line-owned set over the marker corpus (`taskLineCorpus.js`, one line per shape the grammar recognises), so the parser cannot gain a key without the contract failing and the fix is a one-line edit beside the code that caused it, together with the corpus line the same test demands — the residual risk, a conditional key exercised by no corpus line, is closed by the equality (a listed key no line produces fails too), and the corpus is the parser's own fixture so a marker's line joins it where the marker is added; (2) REMOVAL, one test per line-owned key with clearing semantics; (3) THE PROPERTY — a made-up future field survives a re-parse, the test that proves the inversion inverted. Harness scenario 11b pins that the line reported back after a completion changes nothing on the task. **What this does NOT close:** a stale vault copy still re-creates lines the fleet deleted and tombstones lines the fleet added (the day's 24-row resupply storm and a peer's "confirmed removed" on one of its ids were that) — whether a paired phone on a stale heartbeat should scan at all is the open posture ruling in §8, not a field fix.
