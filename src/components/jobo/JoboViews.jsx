...
import { Check, Clock, FileText, Pencil, Trash2, Plus, X, ChevronDown, Save, Undo2, Inbox } from 'lucide-react';
...
function NotesAction({
  ...
  return <button type="button" className="jobo-send-note hover:bg-white/20 rounded p-1 transition-colors" title={t('jobo.addNote')} aria-label={t('jobo.addNote')} onPointerDown={e => e.stopPropagation()} onClick={e => {
    e.stopPropagation();
    onNote ? onNote(task, task._joboRole) : ctx.setExpandedNotesTaskId(task.id);
  }}>
...
function JoboPlanActions({
  ...
  const ctx = useDayPlannerCtx(),
    t = useLang();
  return <>
    <NotesAction task={task} onNote={onNote} />
    <button type="button" className="hover:bg-white/20 rounded p-1 transition-colors" title={t('common.postpone')} onClick={e => {
      e.stopPropagation();
      ctx.postponeTask?.(task.id);
    }}>{icon('undo', 13)}</button>
    <button type="button" className="hover:bg-white/20 rounded p-1 transition-colors" title={t('jobo.edit')} onClick={e => {
      e.stopPropagation();
      ctx.openMobileEditTask?.(task, false);
    }}>{icon('edit', 13)}</button>
    <button type="button" className="hover:bg-white/20 rounded p-1 transition-colors" title={t('task.moveToInbox')} onClick={e => {
      e.stopPropagation();
      ctx.moveToInbox?.(task.id);
    }}><Inbox size={13} /></button>
  </>;
}
  ...
  return <div className="jobo-native-content px-2 py-1 flex-1 min-w-0 h-full flex flex-col">
      <div className="jobo-title-row flex items-center justify-between gap-1">
        ...
        <div className="jobo-native-actions flex items-center gap-0.5 flex-shrink-0">
          {role === 'plan' && !task.imported ? ActionButtons ? <ActionButtons /> : <JoboPlanActions task={task} onNote={onNote} /> : <><NotesAction task={task} onNote={onNote} /><button className="hover:bg-white/20 rounded p-1 transition-colors" title={t('jobo.edit')} onClick={e => {
            e.stopPropagation();
            ctx.openMobileEditTask(task, false);
...
// The latest main TimelineTaskCardContent does not know the prototype's
// compactRenderer prop. Keep that adaptation local to Jobo so the native day,
// week, and mobile cards retain their own renderer and completion semantics.
function JoboTaskContent({
  task,
  height,
  isNarrowWidth,
  onNote
}) {
  if (task._joboRole && !task.imported) return <Compact task={task} onNote={onNote} />;
  return <NativeTaskContent task={task} height={height} isNarrowWidth={isNarrowWidth} />;
}
function LedgerEditor({
  ...
  const ctx = useDayPlannerCtx(),
    t = useLang(),
    state = useLedger();
  // Main exposes task colors as { name, class }; the old prototype used
  // plain Tailwind class strings. Normalize both shapes inside this adapter
  // so the Jobo editor keeps the main palette and stores the task class.
  const colorOptions = (ctx.colors || []).map(color => typeof color === 'string' ? {
    name: color,
    class: color
  } : color).filter(color => color?.class);
  ...
  return <div className="jobo-modal-mask" onMouseDown={e => {
    if (e.target === e.currentTarget) onClose();
      ...
      <label>{t('jobo.recordTitle')}<input autoFocus value={form.title} onChange={e => field('title', e.target.value)} /></label>
      ...
      <div className="flex items-center gap-2 my-2">{colorOptions.map(c => <button type="button" key={c.class} className={`${c.class} jobo-color ${form.color === c.class ? 'ring-2 ring-blue-500' : ''}`} aria-label={c.name || c.class} onClick={() => field('color', c.class)} />)}</div>
...
function Card({
  ...
  const shown = {
    ...
    id = String(task.id),
    mainManagedPlan = role === 'plan' && (task.imported || task.isRecurring || id.startsWith('recurring-')),
    // Imported and recurring plans are owned by main's calendar mutation
    // handlers. Keep them out of the prototype drag path: recurring entries
    // are expanded views of `recurringTasks`, so updating `tasks` by the
    // occurrence id would be a silent no-op and would skip main's exception,
    // undo, and persistence handling.
    prototypeDrag = !(role === 'plan' && mainManagedPlan),
    cardCursor = prototypeDrag ? 'cursor-grab active:cursor-grabbing' : 'cursor-default';
  ...
  const adapted = {
    ...
    toggleComplete: role === 'plan' && !task.imported ? () => {
  ...
  return <div data-jobo-card={role} data-task-id={id} data-jobo-id={`${role}:${id}`} data-ctx-menu draggable={!resize && prototypeDrag} onDragStart={e => {
    if (!prototypeDrag) {
      e.preventDefault();
      return;
    }
  ...
  }} className={`jobo-native-card absolute pointer-events-auto shadow-md notes-panel-container text-white rounded-lg ${cardCursor} ${task.isTaskCalendar ? '' : task.color || 'bg-blue-500'}`} style={{
    top: timeTop,
    height,
    left: `calc(${rect.left * 100}% - ${gutter * rect.left}px + 3px)`,
    width: `calc(${rect.width * 100}% - ${gutter * rect.width + 6}px)`,
    ...calStyle
  }}><DayPlannerContext.Provider value={adapted}><JoboTaskContent task={shown} height={height} isNarrowWidth={false} onNote={onNote} /></DayPlannerContext.Provider>
      {!mainManagedPlan && (!task._joboProjected || C.startEpoch(task) === C.startEpoch(original)) && <div className="jobo-resize-top" aria-label="resize-start" onPointerDown={e => resizeStart(e, 'top')} />}
      {(!task._joboProjected || C.endEpoch(task) === C.endEpoch(original)) && (mainManagedPlan ? <div className="jobo-resize-bottom" aria-label="resize-end" onMouseDown={e => {
        e.stopPropagation();
        ctx.handleResizeStart?.(task, e);
      }}><span /></div> : <div className="jobo-resize-bottom" aria-label="resize-end" onPointerDown={e => resizeStart(e, 'bottom')}><span /></div>)}
    </div>;
}
