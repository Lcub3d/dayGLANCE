...
// one rule is that at least one view stays on. Wherever the app needs a
// view to land on (a persisted default since hidden, a window too narrow
// for the view it was on) it takes the first view still on for that
// width, and only when a width leaves none does it show the list's first
// view regardless.
...
/** Every view, every form factor together (MONTH and SCHED are one view in both switchers). */
export const ALL_VIEWS = ['multi', 'day', 'week', 'grid', 'list', 'month', 'sched'];
...
/**
 * The persisted hidden-views value as a clean list: known views only, no
 * duplicates, in canonical order. A value that would hide every view (a
 * hand-edited one) is ignored: at least one view stays on.
 */
export function normalizeHiddenViews(value) {
  if (!Array.isArray(value)) return [];
  const hidden = ALL_VIEWS.filter((v) => value.includes(v));
  return hidden.length === ALL_VIEWS.length ? [] : hidden;
}
