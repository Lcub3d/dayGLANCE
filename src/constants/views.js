...
// in both (it is the phone's case above all). While the Day Dial is up (the
// kiosk runs it unattended) MONTH is left out of every switcher, so a day
// sheet cannot be opened with nobody there to dismiss it.

export const DESKTOP_VIEW_MODES = ['multi', 'day', 'week', 'sched', 'month'];
/** Narrow desktop and landscape tablet: DAY and WEEK need the 3-column grid. */
export const NARROW_DESKTOP_VIEW_MODES = ['multi', 'sched', 'month'];
export const MOBILE_VIEW_MODES = ['grid', 'list', 'sched', 'month'];
