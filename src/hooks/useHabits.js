...
const useHabits = ({ playUISound, hrOwnerRef }) => {
  ...
  // source:'healthConnect' tag as its sync identity even when the native
  // repository resolves a platform fallback underneath it. Habits synced from
  // the other platform carry the opposite source tag and are intentionally skipped,
      ...
      for (const habit of healthHabits) {
        ...
        try {
          ...
          if (habit.unit === 'steps') {
            ...
            // New Android shells distinguish "no data" / unavailable from a
