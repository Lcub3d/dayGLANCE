...
describe('Jobo local persistence', () => {
  ...
  it('persists Original and recording-time Final Plan separately', () => {
    const {
      S,
      map
    } = load();
    ...
    expect(S.get().plans.p.startTime).toBe('09:00');
    expect(S.get().records[0].planSnapshot).toEqual({
      date: plan.date,
      startTime: '16:00',
      duration: 90
    });
    const restored = load(map.get(LEDGER_KEY)).S.get();
    expect(restored.plans.p.startTime).toBe('09:00');
    expect(restored.records[0].planSnapshot.startTime).toBe('16:00');
  });
  it('reloads a linked Do whose Original Plan is still unknown', () => {
    const {
      S,
      map
    } = load();
    ...
    S.create(task, plan, 'unknown');
    expect(S.get().plans).toEqual({});
    expect(S.get().records[0].planId).toBe('p');
    expect(load(map.get(LEDGER_KEY)).S.get().records[0].planId).toBe('p');
  });
