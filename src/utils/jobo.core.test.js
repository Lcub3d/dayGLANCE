...
test('capture reads main Original Plan and never fabricates it from current schedule', () => {
  const moved = {
    ...p,
    startTime: '16:00',
    duration: 90,
  ...
  const captured = C.capture(C.empty(), [moved]);
  a.equal(captured.plans.p1.startTime, '09:00');
  a.equal(captured.plans.p1.duration, 60);
  a.deepEqual(C.capture(C.empty(), [{
    ...moved,
    originalPlan: undefined
  }]).plans, {});
});
test('Original Plan without duration stays unknown instead of borrowing current duration', () => {
  const task = {
    ...p,
    startTime: '16:00',
    duration: 90,
  ...
  const captured = C.capture(C.empty(), [task]);
  a.equal(captured.plans.p1.duration, undefined);
  a.equal(C.validate(JSON.parse(JSON.stringify(captured))).plans.p1.duration, undefined);
});
...
test('new Do stores the recording-time Final Plan and keeps it immutable', () => {
  const task = {
    ...p,
    startTime: '16:00',
    duration: 90,
  ...
  const s = C.createRecord(C.empty(), task, {
    date: p.date,
    startTime: '16:15',
    duration: 30
  }, 'final', '2026-09-16T03:00:00Z');
  a.deepEqual(s.records[0].planSnapshot, {
    date: p.date,
    startTime: '16:00',
    duration: 90
  });
  a.equal(s.plans.p1.startTime, '09:00');
  a.deepEqual(task, before);
  const moved = C.updateRecord(s, 'final', {
    date: '2026-09-17',
    startTime: '18:00'
  });
  a.deepEqual(moved.records[0].planSnapshot, s.records[0].planSnapshot);
});
test('linked Do remains linked with unknown Original Plan', () => {
  const task = {
    ...p,
    originalPlan: undefined,
    startTime: '16:00',
  ...
  const s = C.createRecord(C.empty(), task, {
    ...
    startTime: '16:15',
    duration: 40
  }, 'unknown-original', '2026-09-16T03:00:00Z');
  a.equal(s.records[0].planId, 'p1');
  a.equal(s.plans.p1, undefined);
  a.deepEqual(C.classifyRecordInState(s, s.records[0]), {
    original: null,
    final: ['delayed'],
    progress: 'complete'
  });
});
test('legacy Do without Final snapshot stays without one after validation', () => {
  const s = record(start());
  delete s.records[0].planSnapshot;
  const restored = C.validate(JSON.parse(JSON.stringify(s)));
  a.equal(Object.prototype.hasOwnProperty.call(restored.records[0], 'planSnapshot'), false);
  a.equal(C.classifyRecordInState(restored, restored.records[0]).final, null);
});
